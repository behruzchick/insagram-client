      ...
      data.map((item) => (
        <Card key={item._id} style={{ width: '500px', height: 'auto' }}>
          <Card.Header className='bg-black'>
            <Link to={user._id === item.user._id ? `/user/profile/${token}` : `/users/profile/${token}/${item.user._id}`} className="user_info" style={{ display: 'flex', gap: '15px', alignItems: 'center', textDecoration: 'none', color: 'white' }}>
